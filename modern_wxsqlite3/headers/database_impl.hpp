...
template <typename _concrete_ty>
bool
database<_concrete_ty>::check_integrity() const
{
  assert(db_ptr_);

  const std::string sql    = "PRAGMA integrity_check";
  sqlite3_stmt* stmt       = nullptr;
  const SQL_RESULT rst_val = sqlite3_prepare_v2(db_ptr_.get(), sql.c_str(),
                                                -1, &stmt, nullptr);
  ...
  {
    if (std::string result = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0)); result != "ok")
    {
      sqlite3_finalize(stmt);
      return false;
    }

...
public:
    ...
    bool used() { return execution_started; }
...
private:
    ...
    sqlite3_stmt* prepare_(const std::string& sql)
    {
        SQL_RESULT ret_code;
        sqlite3_stmt* tmp = nullptr;
        const char* remaining;
        ret_code = sqlite3_prepare_v2(db_.get(), sql.data(), -1, &tmp, &remaining);
    ...
    void extract_singer_value_(std::function<void(void)> callback)
    {
        SQL_RESULT ret_code;
        start_execute_();
    ...
    void extract_multi_values_(std::function<void(void)> callback)
    {
        SQL_RESULT ret_code;
        start_execute_();
...
private:
    ...
    friend void get_column_from_db(database_binder& db, int idx, int& val);
    /* sqlite3_int64 */
    friend database_binder& operator<<(database_binder& db, const sqlite3_int64& val);
    friend void get_column_from_db(database_binder& db, int idx, sqlite3_int64& val);
    /* float */
    friend database_binder& operator<<(database_binder& db, const float& val);
    friend void get_column_from_db(database_binder& db, int idx, float& val);
    /* double */
    friend database_binder& operator<<(database_binder& db, const double& val);
    friend void get_column_from_db(database_binder& db, int idx, double& val);
    /* std::string */
    friend database_binder& operator<<(database_binder& db, const std::string& val);
    friend void get_column_from_db(database_binder& db, int idx, std::string& val);
...
inline
void 
get_column_from_db(database_binder& db, int idx,  int& result)
{
    if(sqlite3_column_type(db.stmt_.get(), idx) == SQLITE_NULL) {
        result = 0;
    }
    else {
        result = sqlite3_column_int(db.stmt_.get(), idx);
    }
}
...
inline
void 
get_column_from_db(database_binder& db, int idx,  sqlite3_int64& result)
{
    if(sqlite3_column_type(db.stmt_.get(), idx) == SQLITE_NULL) {
        result = 0;
    }
    else {
        result = sqlite3_column_int64(db.stmt_.get(), idx);
    }
}
...
inline
void 
get_column_from_db(database_binder& db, int idx, float& f)
{
    if(sqlite3_column_type(db.stmt_.get(), idx) == SQLITE_NULL) {
        f = 0;
    }
    else {
        f = static_cast<float>(sqlite3_column_double(db.stmt_.get(), idx));
    }
}
...
inline
void 
get_column_from_db(database_binder& db, int idx, double& d)
{
    if(sqlite3_column_type(db.stmt_.get(), idx) == SQLITE_NULL) {
        d = 0;
    }
    else {
        d = sqlite3_column_double(db.stmt_.get(), idx);
    }
}
...
inline
void 
get_column_from_db(database_binder& db, int idx, std::string& s)
{
    if(sqlite3_column_type(db.stmt_.get(), idx) == SQLITE_NULL) {
        s = {};
    }
    else {
        s = reinterpret_cast<const char *>(sqlite3_column_text(db.stmt_.get(), idx));
    }
}
